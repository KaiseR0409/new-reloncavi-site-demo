...
const ICONS = [
  { img: "/img/servicios/gruaicon.png", title: "Administración Bodegas Celulosa" },
  { img: "/img/servicios/bodegaicon.png", title: "Ensacados y despacho Fertilizantes" },
  { img: "/img/servicios/barcoicon.png", title: "Estiba y Desestiba portuarias" },
]

const SERVICES = [
  {
    id: "fertilizantes",
    img: "/img/servicios/foto3.webp",
...
export default function Servicios() {
  return (
    <div>
      ...
      <section className="relative bg-cover bg-center bg-fixed py-16 px-6" style={{ backgroundImage: "url(/img/servicios/DJI_0072.webp)" }}>
      ...
      {SERVICES.map((s, idx) => (
        <section id={s.id} key={s.id} className={`py-16 px-6 ${idx % 2 === 0 ? "bg-white" : "bg-white-2"}`}>
          <div className={`max-w-7xl mx-auto grid gap-10 items-center md:grid-cols-2`}>
            <img src={s.img} alt={s.title} loading="lazy" decoding="async" className={`w-full rounded-lg shadow ${idx % 2 === 1 ? "md:order-2" : ""}`} />
            <div>
